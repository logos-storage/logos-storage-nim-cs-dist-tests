...
namespace LogosStorageTests.ScalabilityTests
{
    ...
    public class MultiPeerDownloadTests : AutoBootstrapDistTest
    {
        ...
        {
            var hosts = StartLogosStorage(numberOfHosts, s => s.WithName("host").WithLogLevel(LogosStorageLogLevel.Trace));
            ...
            var blockCids = uploadLog
                ...
                .Select(s =>
                {
                    var line = LogosStorageLogLine.Parse(s)!;
            ...
            var client = StartLogosStorage(s => s.WithName("client").WithLogLevel(LogosStorageLogLevel.Trace));
