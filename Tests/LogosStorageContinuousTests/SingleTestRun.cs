    ...
    public class SingleTestRun
    {
            ...
            StartupChecker startupChecker, CancellationToken cancelToken, string deployId)
        {
            ...
            entryPoint = entryPointFactory.CreateEntryPoint(config.KubeConfigFile, config.DataPath,
                config.LogosStorageDeployment.Metadata.KubeNamespace, fixtureLog);
        ...
        private void DownloadContainerLogs(DateTime testStart)
        {
            ...
            {
                effectiveStart = config.LogosStorageDeployment.Metadata.StartUtc.Subtract(TimeSpan.FromSeconds(30));
            }
        ...
        private IStorageNode[] CreateRandomNodes()
        {
            var instances = SelectRandomInstance();
            fixtureLog.Log("Selected nodes: " + string.Join(",", instances.Select(c => c.Name)));
            return entryPoint.CreateInterface().WrapLogosStorageContainers(instances).ToArray();
        }

        private ILogosStorageInstance[] SelectRandomInstance()
        {
            var number = handle.Test.RequiredNumberOfNodes;
            var instances = config.LogosStorageDeployment.LogosStorageInstances.ToList();
            if (number == -1) return instances.ToArray();

            var result = new ILogosStorageInstance[number];
