...
namespace ContinuousTests
{
    public class StartupChecker
    {
        ...
        public void Check()
        {
            var log = FixtureLog.Create(new LogConfig(config.LogPath), DateTime.UtcNow, config.LogosStorageDeployment.Id,
                "StartupChecks");
            ...
            log.Log("Contacting Logos Storage nodes...");
            CheckLogosStorageNodes(log, config);
            log.Log("All OK.");
        }
        ...
        private void IncludeDeploymentConfiguration(ILog log)
        {
            ...
            //var deployment = config.LogosStorageDeployment;
            //var workflow = entryPoint.Tools.CreateWorkflow();
            //foreach (var instance in deployment.LogosStorageInstances)
            //{
        ...
        private void CheckLogosStorageNodes(ILog log, Configuration config)
        {
            throw new NotImplementedException();

            //var nodes = entryPoint.CreateInterface()
            //    .WrapLogosStorageContainers(config.LogosStorageDeployment.LogosStorageInstances.Select(i => i.Pod).ToArray());
            //var pass = true;
        ...
        private bool EnsureOnline(BaseLog log, IStorageNode n)
        {
            try
            {
                ...
                log.Log($"Logos Storage version: '{info.Version.Version}' revision: '{info.Version.Revision}'");
        ...
        private void CheckRequiredNumberOfNodes(ContinuousTest[] tests, List<string> errors)
        {
            foreach (var test in tests)
            {
                if (test.RequiredNumberOfNodes != -1)
                {
                    ...
                    else if (test.RequiredNumberOfNodes > config.LogosStorageDeployment.LogosStorageInstances.Length)
                    {
                        errors.Add(
                            $"Test '{test.Name}' requires {test.RequiredNumberOfNodes} nodes. Deployment only has {config.LogosStorageDeployment.LogosStorageInstances.Length}");
                    }
                }
            }
        }
